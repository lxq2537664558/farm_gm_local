...
function checktable(issearch){
    $.ajax({
         url:ipaddress+"index.php?m=Agent&c=User&a=itemsList",
         data : {
            "uid" : uid,
            "mid" : mid,
         ...
         success: function (data) {
            ...
            if(data.data){
                for(var i = 0; i<data.data.length; i++){
                    $('#mytable tbody').append(`<tr><td>${data.data[i].itemId}</td><td>${data.data[i].itemName}</td><td>${data.data[i].num}</td></tr>`)
                }
                // 分页以及搜索
                $('#search').show();
                $('#left_page').show();
