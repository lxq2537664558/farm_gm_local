...
for(var i=0; i<$('#mytable tbody tr').length; i++){
	if($('#mytable tbody tr').eq(i).find('td:nth-child(4)').text()==$('#current_number').text()){
		$('#mytable tbody tr').eq(i).find('td:nth-child(5)').text('未提交');
		$('#mytable tbody tr').eq(i).find('td:nth-child(6) button').attr('disabled',false);
		$('#mytable tbody tr').eq(i).find('td:nth-child(6) button').css('background','#00A4FF');
	}else{
		$('#mytable tbody tr').eq(i).find('td:nth-child(5)').text('已提交');
		$('#mytable tbody tr').eq(i).find('td:nth-child(6) button').attr('disabled',true);
		$('#mytable tbody tr').eq(i).find('td:nth-child(6) button').css('background','white');
	}
}
...
$('.delete_worship').click(function(){
	...
	window.location = ipaddress+"index.php?m=Home&c=Basic&a=deleteWorship&area=3&id="+worship_id;
})
...
$('.add_worship').click(function(){
	...
	for(var i = 0; i < $('.input_price').length; i++){
        if($('.input_price').eq(i).val()==""){
            alert("定价不能为空");
            a = false;
            return a;
        }else if(isNaN($('.input_price').eq(i).val())){
            alert("定价需输入正数");
			console.log($('.input_price').eq(i).val().indexOf("-"));
            a = false;
            return a;
        }else if($('.input_price').eq(i).val().indexOf("-") != -1){
			alert("定价需输入正数");
			a = false;
            return a;
		}else{
...
$('#worship_submit').click(function(){
	window.location = ipaddress+"index.php?m=Home&c=Basic&a=worshipSettings&type=3&method=basicWorshipForest&sid="+sid;
})
