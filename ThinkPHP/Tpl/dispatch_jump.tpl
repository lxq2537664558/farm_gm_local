...
.content{width:450px;font-size:32px;margin-left:8%;text-align:center;}
</style>
</head>.
<body>
	<div class="wrap <?php if(empty($status)) echo 'error'; ?>" style="width: 500px;height: 300px;">
		<span class="bigico"></span>
		<div class="content">
			<?php if(isset($message)) echo $message;if(isset($error)) echo $error;  ?>
			<div style="font-size: 40px;margin-top: 10px;text-align: center;">
				<span id="J_timeleft"><?php echo($waitSecond); ?></span>
				秒后页面自动<br/>
				<a id="J_href" href="<?php echo($jumpUrl); ?>">跳转页面</a>
			</div>
		</div>
	</div>
	<span class="vam"></span>
